...
abstract class SqlTemplate extends BaseTemplate
{
    ...
    {return $this->sorgu;}
    protected function setSorgu($sorgu)
    {$this->sorgu = $sorgu;}
...
abstract class MySqlKayitYapTemplate extends SqlBindTemplate
{
    ...
    public function __construct()
    {
        ...
        //parent::__construct("SELECT * FROM kayitlar WHERE mail=?");
    ...
    protected function veritabaniSorguData()
    {
        ...
        if ($kontrolArray[0]) {
            ...
            $this->setSonucDiziEleman("kullaniciEklemePostArray", $array);

            $dizi[] = "mehmet@katı.com";

            $this->setSonucDiziAjaxEleman("diziİcerigi", $dizi[0]);
            return $array;
        }
    ...
    {
        $this->setSonucDiziEleman("sorguDurum", $this->getDb()->getSorguDurumu());
        /* if ($this->getDb()->getDataSayisi() == 0) {
    /* $this->setSorgu("INSERT INTO kayitlar(kullaniciAd,kullaniciSoyAd,mail,sifre) VALUES (?,?,?,?)");
    $this->veritaniIslem($this->getSorgu(), $this->getSonucDiziEleman("kullaniciEklemePostArray"));
    $this->setSonucDiziEleman("sorguDurum", $this->getDb()->getSorguDurumu());
    $this->setSonucDiziEleman("data", $this->getDb()->getData());
    } else {
    $this->setSonucDiziEleman("sorguDurum", false);
    }*/
